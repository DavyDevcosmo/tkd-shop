...
const ProductFilters = () => {
    return (
        <aside className="w-64 p-4 text-gray-800 rounded-lg shadow-lg"> {/* Removido bg-gray-900 */}
            <h2 className="text-2xl font-bold mb-6 text-yellow-500">Filtrar por</h2>

            {/* Filtro por Cor */}
            <div className="mb-6 border-b border-gray-300 pb-4"> {/* Cor da borda ajustada */}
                <h3 className="text-lg font-semibold mb-3">Cor</h3>
                <ul className="space-y-2">
                    {['Cinza (1)', 'Laranja (1)', 'Marfim (1)', 'Marrom (1)', 'Preto (1)', 'Amarela (1)', 'Azul Clara (1)', 'Azul Escura (1)'].map((color) => (
                        <li key={color}>
                            <label className="inline-flex items-center cursor-pointer">
                                <input
                                    type="checkbox"
                                    // Ajustado bg-white e border-gray-400 para o checkbox ficar bom em fundo claro
                                ...
                                />
                                <span className="ml-2 text-gray-800 text-sm">{color}</span> {/* Cor do texto ajustada */}
                            </label>
                        </li>
                    ))}
            ...
            <div className="mb-6 border-b border-gray-300 pb-4"> {/* Cor da borda ajustada */}
                ...
                <ul className="space-y-2">
                    {['Ac (1)', 'Al (1)', 'Am (1)', 'Ap (1)', 'Ba (1)', 'Ce (1)', 'Df (1)', 'Es (1)'].map((state) => (
                        <li key={state}>
                            <label className="inline-flex items-center cursor-pointer">
                                <input
                                    type="checkbox"
                                    className="form-checkbox h-5 w-5 text-yellow-500 bg-white border-gray-400 rounded focus:ring-yellow-500 transition duration-150 ease-in-out checked:bg-yellow-500 checked:border-yellow-500"
                                />
                                <span className="ml-2 text-gray-800 text-sm">{state}</span>
                            </label>
                        </li>
                    ))}
                    <li className="text-yellow-500 hover:underline cursor-pointer text-sm mt-2">Ver todos</li>
            ...
            <div className="mb-4">
                ...
                <div className="flex space-x-3">
                    <div className="flex flex-col flex-1">
                        <label htmlFor="priceFrom" className="text-sm mb-1 text-gray-600">De</label> {/* Cor do texto ajustada */}
                        <input
                            type="number"
                            id="priceFrom"
                            placeholder="0"
                            // Ajustado bg-gray-100 e border-gray-300 para input ficar bom em fundo claro
                    ...
                    <div className="flex flex-col flex-1">
                        <label htmlFor="priceTo" className="text-sm mb-1 text-gray-600">Até</label> {/* Cor do texto ajustada */}
                        <input
                            type="number"
                            id="priceTo"
                            placeholder="9999"
